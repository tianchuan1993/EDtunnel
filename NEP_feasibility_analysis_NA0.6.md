# 指标分析计算一：NEP ≤100 fW/√Hz 可行性分析（NA=0.6 修正版）

本文档对原始 NEP 可行性分析进行审核校正，并以 **NA = 0.6** 重新计算全部链路参数。
...
| --- | --- | --- |
| 正文推导（arcsin 部分） | ≈0.07 | 0.12% |
| 汇总表标题 | 0.25 | — |
| 汇总表中光子率（步骤⑤⑥） | 0.25（隐含） | 1.59% |
| 灵敏度裕度分析基准值 | 0.25 | 1.59% |
| 灵敏度裕度 NEP 基准 | 0.25 | 对应 NEP=0.883 fW/√Hz |
| 正文最终 NEP | ≈0.07 | 对应 NEP=3.2 fW/√Hz |

**结论**：正文推导过程与汇总表/裕度分析使用了不同的 NA 值，导致 NEP 结果矛盾（3.2 vs 0.883 fW/√Hz）。

### 2. arcsin 参数笔误

正文写 $\theta = \arcsin(0.06) = 4.01°$，但 $\arcsin(0.06) = 3.44°$。实际 $\arcsin(0.07) = 4.01°$，说明 NA=0.07，书写时误写为 0.06。
...
以下保持原文其他参数不变，仅将荧光收集 NA 更新为 **0.6**。
...
### 2.4 THz 光子 → 527 nm 光子 → CCD 光电子（NA=0.6 更新）
...
$$
\theta = \arcsin(0.6) = 36.87°
$$

$$
\Omega = 2\pi(1 - \cos\theta) = 2\pi(1 - 0.800) = 2\pi \times 0.200 = 1.257\ \text{sr}
$$

$$
\eta_{\text{collect}} = \frac{\Omega}{4\pi} = \frac{1.257}{12.566} = 10.0\%
$$
...
$$
\dot{N}_{\text{CCD}} = \dot{N}_{527} \times \eta_{\text{collect}} \times T_{\text{optics}} = 1.315 \times 10^8 \times 0.100 \times 0.851 = 1.119 \times 10^7\ \text{photons/s}
$$

**CCD 光电子产生率**（QE = 72%）🔄 更新：

$$
\dot{N}_{e^-} = \dot{N}_{\text{CCD}} \times QE = 1.119 \times 10^7 \times 0.72 = 8.06 \times 10^6\ e^-/\text{s}
$$

### 2.5 散粒噪声极限 NEP（NA=0.6 更新）

散粒噪声主导时，$1\ \text{Hz}$ 带宽内信噪比：

$$
SNR_{1\text{Hz}} = \sqrt{\dot{N}_{e^-}} = \sqrt{8.06 \times 10^6} = 2839
$$

NEP：

$$
\boxed{NEP = \frac{P_{\text{THz}}}{SNR_{1\text{Hz}}} = \frac{1 \times 10^{-12}}{2839} = 3.52 \times 10^{-16}\ \text{W/}\sqrt{\text{Hz}} = 0.352\ \text{fW/}\sqrt{\text{Hz}}}
$$

$$
NEP = 0.352\ \text{fW/}\sqrt{\text{Hz}} \ll 100\ \text{fW/}\sqrt{\text{Hz}} \quad \checkmark
$$
...
| ④ | 荧光收集（NA=0.6） | $\eta_{\text{collect}} = 10.0\%$ | $1.315 \times 10^7\ /\text{s}$ |
| ⑤ | 光学透射 | $T_{\text{optics}} = 85.1\%$ | $1.119 \times 10^7\ /\text{s}$ |
| ⑥ | CCD QE @527 nm | $QE = 72\%$ | $8.06 \times 10^6\ e^-/\text{s}$ |
...
$$
= 0.05 \times 0.557 \times 0.100 \times 0.851 \times 0.72 = 1.71 \times 10^{-3}
$$

即每约 **585** 个 THz 光子产生 1 个 CCD 光电子。
...
| 收集 NA | 0.6 | 0.5 | $\times\sqrt{10.0/6.70} = \times 1.22$ |
| 光学透射率 | 85.1% | 75% | $\times\sqrt{85.1/75} = \times 1.07$ |
| QE @527 nm | 72% | 60% | $\times\sqrt{72/60} = \times 1.10$ |
| **全部最差叠加** | — | — | **$\times 2.39$** |

> NA=0.5 对应 $\theta = 30°$，$\Omega = 2\pi(1 - \cos 30°) = 2\pi \times 0.134 = 0.842\ \text{sr}$，$\eta_{\text{collect}} = 0.842/12.566 = 6.70\%$。

**最差情况 NEP**：

$$
0.352 \times 2.39 = 0.841\ \text{fW/}\sqrt{\text{Hz}} \ll 100\ \text{fW/}\sqrt{\text{Hz}}
$$
...
| 对比项 | 原文 (NA≈0.07) | 原文表 (NA=0.25) | 本文 (NA=0.6) |
| --- | --- | --- | --- |
| 收集半角 $\theta$ | 4.01° | 14.48° | 36.87° |
| 立体角 $\Omega$ | 0.0154 sr | 0.200 sr | 1.257 sr |
| 收集效率 $\eta_{\text{collect}}$ | 0.12% | 1.59% | 10.0% |
| CCD 光电子率 | $9.65 \times 10^4\ e^-/\text{s}$ | $1.28 \times 10^6\ e^-/\text{s}$ | $8.06 \times 10^6\ e^-/\text{s}$ |
| NEP | $3.2\ \text{fW/}\sqrt{\text{Hz}}$ | $0.883\ \text{fW/}\sqrt{\text{Hz}}$ | $0.352\ \text{fW/}\sqrt{\text{Hz}}$ |
| 最差情况 NEP | — | $2.16\ \text{fW/}\sqrt{\text{Hz}}$* | $0.841\ \text{fW/}\sqrt{\text{Hz}}$ |
| 对 100 fW/√Hz 裕度 | ~31× | ~46× | ~284× (最差 ~119×) |

> \*原文标注为 1.94 fW/√Hz，但其组合因子 ×2.20 有误，校正为 ×2.45 后实为 2.16 fW/√Hz。
...
2. **主要问题是 NA 取值不一致**：正文推导用 NA≈0.07，汇总表和灵敏度分析用 NA=0.25，导致 NEP 结果自相矛盾。
3. **以 NA=0.6 重新计算后**：
   - 收集效率由 0.12%（NA=0.07）大幅提升至 **10.0%**（NA=0.6）；
   - 系统总效率 $\eta_{\text{total}} = 1.71 \times 10^{-3}$，每约 585 个 THz 光子产生 1 个 CCD 光电子；
   - **NEP = 0.352 fW/√Hz**，远低于 100 fW/√Hz 指标；
   - 即使全部参数取最差值，NEP = 0.841 fW/√Hz，仍有 **超过 100 倍裕度**，指标可靠满足。
